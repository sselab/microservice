...
def readExcel (filename, col_name_index = 0, by_index = 0) :
    ...
    # empirical time complexity of the microservice
    f_a = numpy.matrix(table.col_values(col_Name.index('f(\xce\xb1)'.decode('utf-8'))))
    f_a = f_a[:, 1:].T
    f_a = numpy.mat([int(x) for x in f_a]).T

    # number of concurrent threads
    x1 = numpy.matrix(table.col_values(col_Name.index('x')))
    x1 = x1[:, 1:].T
    x1 = numpy.mat([int(i) for i in x1]).T

    # time interval
    t_2 = numpy.matrix(table.col_values(col_Name.index('t3')))
    t_2 = t_2[:, 1:].T
    ...
    # the data set
    xMat = numpy.hstack((constant, f_a, x1, c, r_t, b_t))
    yMat = t
...
def lwlr(testPoint, xMat, yMat, k=0.1):     # k controls the decay rate
